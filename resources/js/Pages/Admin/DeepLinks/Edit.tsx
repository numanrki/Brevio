...
import { DeepLink, Pixel } from '@/types';
...
export default function Edit({ deepLink, pixels }: Props) {
    const { data, setData, put, processing, errors } = useForm<{
        ...
        rules: (deepLink.rules || []).map((r) => ({ type: r.type, value: r.value, destination_url: r.destination_url })),
        pixel_ids: (deepLink.pixels || []).map((p) => p.id),
    });
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        put(url(`/admin/deep-links/${deepLink.id}`));
    };

    return (
        <AdminLayout header="Edit Deep Link">
            <Head title="Edit Deep Link" />
                ...
                <form onSubmit={submit} className="space-y-6">
                    ...
                    <div className="rounded-xl bg-gray-900 border border-gray-800 p-6 space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} className="w-full px-4 py-2.5 bg-gray-950 border border-gray-800 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/40" />
                            {errors.name && <p className="text-xs text-red-400 mt-1">{errors.name}</p>}
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1.5">Alias</label>
                            <div className="px-4 py-2.5 bg-gray-950 border border-gray-800 rounded-xl text-sm text-gray-500">
                                <code>{deepLink.alias}</code>
                        ...
                        <div>
                            ...
                            <input type="url" value={data.fallback_url} onChange={(e) => setData('fallback_url', e.target.value)} className="w-full px-4 py-2.5 bg-gray-950 border border-gray-800 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/40" />
                    ...
                    <div className="rounded-xl bg-gray-900 border border-gray-800 p-6 space-y-5">
                        ...
                        {data.rules.length === 0 && (
                            <p className="text-sm text-gray-500">No rules. All visitors will be sent to the fallback URL.</p>
                        )}
