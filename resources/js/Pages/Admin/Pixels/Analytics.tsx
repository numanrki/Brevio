...
import { Pixel, TimeSeriesPoint, BreakdownItem } from '@/types';
...
interface Props {
    ...
    custom_from: string;
    custom_to: string;
    summary: { total_fires: number; unique_fires: number; avg_daily: number };
    fires_over_time: TimeSeriesPoint[];
...
export default function Analytics({ pixel, range, ranges, custom_from, custom_to, summary, fires_over_time, top_countries, top_browsers, devices, visitor_log }: Props) {
    ...
    const changeRange = (newRange: string) => {
        ...
        setShowCustom(false);
        router.get(url(`/admin/pixels/${pixel.id}/analytics`), { range: newRange }, { preserveScroll: true });
    };
    const applyCustomRange = () => {
        router.get(url(`/admin/pixels/${pixel.id}/analytics`), { range: 'custom', from: dateFrom, to: dateTo }, { preserveScroll: true });
    };

    return (
        <AdminLayout header="Pixel Analytics">
            <Head title={`Analytics: ${pixel.name}`} />

            <div className="max-w-6xl">
                <Link href={url(`/admin/pixels/${pixel.id}`)} className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-white mb-4 transition-colors">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-lg font-semibold text-white">{pixel.name}</h2>
                        <p className="text-sm text-gray-500 font-mono">{pixel.token}</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-6 border-b border-gray-800 pb-3">
                    ...
                    <button onClick={() => setTab('visitors')} className={`text-sm font-medium pb-1 transition-colors ${tab === 'visitors' ? 'text-violet-400 border-b-2 border-violet-400' : 'text-gray-500 hover:text-white'}`}>Fire Log</button>
                </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <div className="rounded-xl bg-gray-900 border border-gray-800 p-5">
                                ...
                                <p className="text-2xl font-bold text-white">{summary.total_fires.toLocaleString()}</p>
                            </div>
                            <div className="rounded-xl bg-gray-900 border border-gray-800 p-5">
                                <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Unique Fires</p>
                                <p className="text-2xl font-bold text-white">{summary.unique_fires.toLocaleString()}</p>
                            </div>
                            <div className="rounded-xl bg-gray-900 border border-gray-800 p-5">
                        ...
                        <div className="rounded-xl bg-gray-900 border border-gray-800 p-6 mb-6">
                            <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider mb-4">Fires Over Time</h3>
                            {fires_over_time.length > 0 ? (
                                <ResponsiveContainer width="100%" height={250}>
                                    <LineChart data={fires_over_time}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="rounded-xl bg-gray-900 border border-gray-800 p-6 md:col-span-2">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-800/50">
                                {visitor_log.length === 0 ? (
                                    <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">No fires in this period.</td></tr>
                                ) : visitor_log.map((v) => (
                                    <tr key={v.id} className="hover:bg-gray-900/40">
                                        ...
                                        <td className="px-4 py-2.5 text-gray-400 truncate max-w-[200px]">{v.referrer || '—'}</td>
