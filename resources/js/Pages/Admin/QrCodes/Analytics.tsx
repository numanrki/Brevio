...
interface Props {
    qrCode: { id: number; name: string; scans: number; url?: { alias: string } | null };
    range: string;
    ranges: string[];
    custom_from: string;
    custom_to: string;
...
export default function Analytics({ qrCode, range, ranges, custom_from, custom_to, summary, visits_over_time, top_countries, top_cities, top_browsers, top_os, devices, top_referrers, visitor_log }: Props) {
    ...
    const changeRange = (newRange: string) => {
        if (newRange === 'custom') {
            router.get(url(`/admin/qr-codes/${qrCode.id}/analytics`), { range: 'custom', from: dateFrom, to: dateTo }, { preserveState: true, preserveScroll: true });
        } else {
            router.get(url(`/admin/qr-codes/${qrCode.id}/analytics`), { range: newRange }, { preserveState: true, preserveScroll: true });
        }
    };

    return (
        <AdminLayout header="QR Code Analytics">
            <Head title={`Analytics: ${qrCode.name}`} />

            <div className="max-w-6xl">
                <Link href={url('/admin/qr-codes')} className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-white mb-4 transition-colors">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-lg font-bold text-white">{qrCode.name}</h2>
                        {qrCode.url && <p className="text-sm text-gray-500">Linked to /{qrCode.url.alias}</p>}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <SummaryCard label="Total Scans" value={summary.total_clicks} />
                    <SummaryCard label="Unique Scans" value={summary.unique_clicks} />
                ...
                {tab === 'overview' && (
                    <>
                        ...
                        <div className="rounded-xl bg-gray-900 border border-gray-800 p-6 mb-6">
                            <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Scans Over Time</h3>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-800/50">
                                    {visitor_log.length === 0 ? (
                                        <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-500">No scans recorded yet</td></tr>
                                    ) : (
                                        visitor_log.map((v, i) => (
