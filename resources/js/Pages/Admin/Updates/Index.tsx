...
export default function Index({ currentVersion, lastCheck, pendingMigrations: initialPending }: Props) {
    const [channel, setChannel] = useState<Channel>('stable');
    const [state, setState] = useState<PageState>('idle');
    const [errorMessage, setErrorMessage] = useState('');
    const [pendingMigrations, setPendingMigrations] = useState(initialPending);
    const [migratingState, setMigratingState] = useState<'idle' | 'running' | 'done' | 'error'>('idle');
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-amber-400">
                                    {pendingMigrations} Pending Database Migration{pendingMigrations !== 1 ? 's' : ''}
                                </p>
                                <p className="text-xs text-amber-400/70 mt-0.5">
                                    New database changes were found that haven't been applied yet. Run migrations to update your database.
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={async () => {
                                setMigratingState('running');
                                setMigrationResult('');
                                try {
                                    const data = await apiCall('/admin/updates/run-migrations');
                                    setMigrationResult(data.output || 'No new migrations.');
                                    setPendingMigrations(data.pending_migrations ?? 0);
                                    setMigratingState('done');
                                } catch (err: unknown) {
                                    setMigrationResult(err instanceof Error ? err.message : 'Migration failed');
                                    setMigratingState('error');
                                }
                            }}
                            disabled={migratingState === 'running'}
                            className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 disabled:opacity-50 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-amber-500/20 flex-shrink-0"
                        >
                            {migratingState === 'running' ? (
                            ...
                            {migratingState === 'running' ? 'Running...' : 'Run Migrations'}
                        </button>
                    </div>
                    {migrationResult && (
                        <pre className={`mt-4 text-xs rounded-lg p-3 max-h-40 overflow-auto ${
                            migratingState === 'error'
                                ? 'bg-red-500/10 border border-red-500/20 text-red-400'
                                : 'bg-gray-950 text-gray-400'
                        }`}>{migrationResult}</pre>
                    )}
                    {migratingState === 'done' && pendingMigrations === 0 && (
                    ...
                    {release && (
                        <div className="rounded-xl bg-gray-900 border border-gray-800 overflow-hidden">
                            ...
                            <div className="p-6">
                                ...
                                {!hasStableUpdate && (
                                    <div className="flex items-center gap-3 mb-6 p-4 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                                        <svg className="w-5 h-5 text-emerald-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        <p className="text-sm text-emerald-400">You're running the latest stable release. No updates available.</p>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-3">
                                    {hasStableUpdate && (
                                        <button
                                            onClick={() => installUpdate('/admin/updates/install-stable', { version: release.version, zipball_url: release.zipball_url })}
                                            className="inline-flex items-center gap-2 px-5 py-2.5 text-white text-sm font-medium rounded-xl transition-all shadow-lg bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 shadow-emerald-500/20"
                                        >
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <div className="rounded-xl bg-gray-900 border border-gray-800 overflow-hidden">
                            ...
                            <div className="divide-y divide-gray-800/50 max-h-[600px] overflow-auto">
                                {commits.map((commit, idx) => (
                                    <div key={commit.sha} className={`px-6 py-4 hover:bg-gray-800/30 transition-colors ${idx === 0 && !isBetaUpToDate ? 'bg-amber-500/5' : idx === 0 && isBetaUpToDate ? 'bg-emerald-500/5' : ''}`}>
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    {idx === 0 && isBetaUpToDate && (
                                                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase flex-shrink-0">Installed</span>
                                                    )}
                                                    {idx === 0 && !isBetaUpToDate && (
